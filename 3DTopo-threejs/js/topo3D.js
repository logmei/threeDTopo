...
// 默认内置方法。
   ...
   TopoClass.pt.config = {
    ...
    TopoClass.pt.drawDDosDevices=function(){//画受攻击的设备
        for(var i=0;i<ddos.length;i++){
            for(var j=0;j<reportMap.elements.length;j++){
                var node=reportMap.elements[j];
                if(ddos[i].ip==node.deviceName||ddos[i].ip==node.mgmtIp){
                   this.drawDDosDevice(node.deviceId);
                }
            }

        }
    },
    TopoClass.pt.drawDDosDevice=function(deviceId){//画受攻击的设备
        var node=reportMap.get(deviceId);
        for(var i=0;i<ddos.length;i++){
            if(ddos[i].ip==node.deviceName||ddos[i].ip==node.mgmtIp){
    ...
    TopoClass.pt.drawconfigCheckDevices=function(){//画配置核查的设备
        for(var i=0;i<configCheck.length;i++){
        	 for(var j=0;j<reportMap.elements.length;j++){
        		 var node=reportMap.elements[j];
	            if(configCheck[i].ip==node.mgmtIp){
	               this.drawconfigCheckDevice(node.deviceId);
	            }
        	 }
        }
       
    }
    TopoClass.pt.drawconfigCheckDevice=function(deviceId){//画配置核查的设备
        var node=reportMap.get(deviceId);
        var had=false;
        var check=[];
        for(var i=0;i<configCheck.length;i++){
