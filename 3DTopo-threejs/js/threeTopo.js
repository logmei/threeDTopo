(function (global, factory) {
    ...
    var params={
        ...
        pointCircle:{
          ...
          StateTypes:{
             Identify:'Circle',
             Alarm:'Cube'//Circle:旋转圆环，Cube：正方体
           },
         relations:{
             rotation:'Circle',
             changeColor:'Cube'
   ...
   var orthoCam;
    ...
    var linematerial = new THREE.MeshPhongMaterial( {color: 0x156289,
        ...
        side: THREE.DoubleSide,
        shading: THREE.FlatShading  } );
    var linematerial_in = new THREE.MeshPhongMaterial( {color: 0xE33539,
        emissive: 0xE33539,
        side: THREE.DoubleSide,
        shading: THREE.FlatShading  } );
    ...
    function TopoClass(options){
        ...
        this.controls={
            ...
            alarmStateType:params.pointCircle.alarmStateType,
            //摄像机的位置
            DefaultAngle: this.defaultVisibleAngle,//默认视角

            cameraX:params.camera.position.x,
            cameraY:params.camera.position.y,
            cameraZ:options.cameraZ,
            //弹出框个数
            reportTipNumber:params.reportTipNumber
...
*/
           ...
           // renderer.shadowMap.enabled = true;
            ...
            circleState=this.LoadSelectState(params.pointCircle.loadStateType);//加载选中状态
            ...
            function onDocumentMouseDown( event ) {
                ...
                raycaster.setFromCamera( mouse, camera );
                    ...
                    if ( intersects.length > 0 && FilterNoRaycasterObject(intersects[ 0 ].object) && intersects[ 0 ].object.parent!=undefined) {
                        ...
                        if(intersect.name!=null&&intersect.name.indexOf("Node_")==-1&&intersect.name!="selectState")intersect = intersects[ 0 ].object.parent.parent;
                        if(intersect!=null&&intersect.name!=null&&(intersect.name.indexOf("Node_")!=-1||(intersect.name=="selectState"&&intersect.deviceId!=undefined))) {
                            var deviceId=getDeviceId(intersect);
                            if(deviceId==null)return;
                            if(isCtrlDown){//按住ctrl键进行选择
                                if(scene.getObjectByProperty("deviceId","selectState_Identify_Node_"+deviceId)!=undefined)topo.removeCheckedIdentify(deviceId);
                                else topo.paintingCheckedIdentify(deviceId);
                            }else{//查看报告
                                reportOfDevice.showTip(deviceId);
                                if(scene.getObjectByProperty('deviceId',"selectState_Identify_Node_"+intersect.name)==undefined) topo.paintingCheckedIdentify(deviceId);
                            }
                        }
                        if(intersect!=null&&intersect.name!=null&&intersect.name.indexOf("Line-")!=-1) {
                               reportOfDevice.showLinkTip(intersect.name);
                        }
                        render();
            ...
            function getDeviceId(obj){
                var name=obj.name;
                var deviceId="";
                if(name.indexOf("Node_")!=-1)return name.split("Node_")[1];
                else if(name=="selectState"&&obj.deviceId!=undefined){
                    return obj.deviceId.split("Node_")[1];
                }else if(name=="selectState"&&obj.deviceId==undefined){
                    return null;
                }
            }
            ...
            function render() {
                ...
                //if(that.controls.DefaultAngle)that.defaultVisibleAngle();
                camera.updateMatrixWorld();

                if(params.camera.Helper)that.InitHelp();


                scene.traverse(function(e){
                    //选中状态
                    if( e.name=="selectState"||e.name=="selectStateAlarm"){
                        if(e.stateType==params.pointCircle.relations.rotation) e.DynamicRotate();
                        if(e.stateType==params.pointCircle.relations.changeColor) e.ChangeColor();
                    }
                    if(e.name=="DisconnectIdentifyState"){
                        e.rotation.x+=params.disconnectState.rotateSpeed;
                        e.rotation.y+=params.disconnectState.rotateSpeed;
                        e.rotation.z+=params.disconnectState.rotateSpeed;
                    }
                });
                raycaster.setFromCamera(mouse,camera);
                var intersects = raycaster.intersectObjects( scene.children,true);
                if(!isCtrlDown) {//按住ctrl键只进行选择
                    if (intersects.length > 0 && FilterNoRaycasterObject(intersects[ 0 ].object) && intersects[ 0 ].object.parent != undefined) {
                        if (INTERSECTED != intersects[ 0 ].object.parent.parent && intersects[ 0 ].object.parent.parent != null) {
                            ...
                            if (INTERSECTED.name != null && (INTERSECTED.name.indexOf("Node_") != -1||(INTERSECTED.name=="selectState"&&INTERSECTED.deviceId!=undefined))) {
                                var deviceId=getDeviceId(INTERSECTED);
                                circleState.visible = true;
                                circleState.deviceId="selectState_Node_"+deviceId;
                                circleState.position.x = INTERSECTED.position.x;
                                circleState.position.y = INTERSECTED.position.y - params.pointCircle.positionDown;
                                circleState.position.z = INTERSECTED.position.z;
                            }
                        ...
                        circleState.visible = false;
                        cubeState.visible = false;
                        delete circleState.deviceId;
                         INTERSECTED = null
                    }
                }
                ...
                if(params.camera.Helper)that.ViewCameraHelp();
        ...
        InitHelp:function(){
            var left   = Math.floor(  window.innerWidth  * 0.0 );
            var bottom = Math.floor(  window.innerHeight * 0.5 );
        ...
        reSetCameraPosition: function(type,value){
            camera.position[type] = value;
            var planePos=new THREE.Vector3(0,0,0);
            orbitControls.target=planePos.clone();//改变视角必填项
            camera.lookAt(planePos);//改变视角必填项
            camera.updateMatrixWorld();//改变视角必填项
        },
        CreateAxes : function(){
        ...
        defaultVisibleAngle:function(){//默认视角
        ...
        //加载选中状态
        LoadSelectState:function(stateType){
            var selectState = new SelectStatesClass();
            selectState.name="selectState";
            scene.add( selectState );
            //选中注释
            switch(stateType){
                case "Circle":
                    selectState.visible=false;
                    selectState.position.y = 5;
                    selectState.rotation.x=params.plane.rotationX;
                    selectState.addCircleState(this);
                    break;
                case "Cube":
                    selectState.visible=false;
                    selectState.addCubeState(this);
                    break;
            }
            return selectState;
        },
    InitGUI:function(){
        ...
        gui.add(this.controls,'DefaultAngle');
        ...
        gui.add(this.controls,'reportTipNumber').min(1).step(1).onChange(function(){//设置弹出框的个数
            reportOfDevice.setTipNumber();
        });
        var that=this;
        ...
        equiment.add(this.controls,'layerChange',-1,1).step(1).onChange(function(value){
            ...
            if((value==1||value==0||value==-1)&&lastchangeVal!=value){
                ...
                for(var i=0;i<checkArr.length;i++){
                    ...
                    var checkObj=scene.getObjectByProperty("deviceId","selectState_Identify_Node_"+checkArr[i]);
                    changeHight(obj,checkObj);
                }

                //报警
                for(var i=0;i<AlarmArr.length;i++){
                    var obj= scene.getObjectByName("Node_"+AlarmArr[i]);
                    var checkObj=scene.getObjectByProperty("deviceId","selectState_Alarm_Node_"+AlarmArr[i]);
                    changeHight(obj,checkObj);
                }
            }
            lastchangeVal=value;
             ...
             * @param stateObj
             */
            function changeHight(obj,stateObj){
                if(value>0) {
        ...
        var IdentifyStates=gui.addFolder('Identify-States');
        var selectStates=IdentifyStates.addFolder("Select-States");
      ...
      UnitVector : function(nodes1,nodes2){//单位向量
          var x=nodes1.x-nodes2.x;
          var y=nodes1.y-nodes2.y;
          var z=nodes1.z-nodes2.z;
       ...
       ReturnNode : function(nodes1,nodes2){
           var unitVector=this.UnitVector(nodes1,nodes2);
           var node=new Array();
           var node1=new THREE.Vector3();
           node1.x=nodes1.x-3*unitVector.z;
           node1.z=nodes1.z-3*unitVector.x;
           node1.y=nodes1.y;
           var node2=new THREE.Vector3();
           node2.x=nodes2.x-3*unitVector.z;
           node2.z=nodes2.z-3*unitVector.x;
           node2.y=nodes2.y;
           node.push(node1);
           node.push(node2);
           return node;
    ...
    PaintingLinks:function(){
        for(var i=0;i<this.Links.length;i++){
            ...
            var node=reportMap.get(link.deviceId);
            if(node==undefined)continue;
            node1.ip=node.mgmtIp==""?node.deviceName:node.mgmtIp;
            node1.deviceName=node.deviceName;

            node1.deviceId=link.deviceId;
            node1.interfaceName=link.interfaceName;
            node1.fromName="源";


            var node2=new Object();
            node=reportMap.get(link.nbDeviceId);
            if(node==undefined)continue;
            node2.ip=node.mgmtIp==""?node.deviceName:node.mgmtIp;
            node2.deviceName=node.deviceName;
            node2.deviceId=link.nbDeviceId;
            node2.interfaceName=link.nbInterfaceName;

            node2.fromName="目的";
            LinkMap.put(lineId,[node1,node2]);
        }
    },
        /**
         * 画节点
       ...
       paintingCheckedIdentify:function(deviceId){
           var state=scene.getObjectByProperty("deviceId","selectState_Identify_Node_"+deviceId);
           if(state!=undefined){
           ...
           for(var i=0;i<selected.children.length;i++){
               selected.children[i].material.color=new THREE.Color( this.controls.visitedColor );
           }
           selected.deviceId="selectState_Identify_Node_"+deviceId
           var pos=scene.getObjectByName("Node_"+deviceId).position;
        ...
        //画报警标志
        paintingAlarmState:function(deviceId){
            var state=scene.getObjectByProperty("deviceId","selectState_Alarm_Node_"+deviceId);
            if(state!=undefined){
                if(!state.visible)state.visible=true;
                // state.position.y+=params.pointCircle.positionDown;
                return;
            }
            var selected = new SelectStatesClass();
            selected.addCubeState();
            selected.name="selectStateAlarm";
            scene.add( selected );
            selected.deviceId="selectState_Alarm_Node_"+deviceId
            var pos=scene.getObjectByName("Node_"+deviceId).position;
            selected.position.x=pos.x;
            selected.position.z=pos.z;
            selected.position.y=pos.y;
            selected.visible=true;
            AlarmArr.push(deviceId);
        },
        paintingDisconnectIdentify:function(position){
           ...
           var cubeMesh = new THREE.Mesh(geometry, material);
            ...
            cubeMesh.name="DisconnectIdentifyState";
            scene.add( cubeMesh );
            material.map    = new THREE.TextureLoader().load('modules/images/disconnect.jpg')
        },
        //删除选中状态的标志
       removeCheckedIdentify:function(deviceId){
            var stat=scene.getObjectByProperty('deviceId',"selectState_Identify_Node_"+deviceId);
           scene.remove(stat);
           for(var i=0;i<checkArr.length;i++){
        ...
        //删除报警的标志
        removeAlarmState:function(deviceId){
            var stat=scene.getObjectByProperty('deviceId',"selectState_Alarm_Node_"+deviceId);
            scene.remove(stat);
            for(var i=0;i<AlarmArr.length;i++){
                if(deviceId==AlarmArr[i])AlarmArr.splice(i, 1);
            }
        },
    ...
    PointingEarth:function(){
        ...
        scene.add(earthMesh)
        //2、
        material.map    = new THREE.TextureLoader().load('modules/images/earth/earthmap1k.jpg')
        //3、
...
// create the mesh based on geometry and material
    ...
    AddTube : function(nodes,lineId){
        ...
        centerNode.x=((nodes[1].x-nodes[0].x)/2)+nodes[0].x;
        centerNode.y=((nodes[1].y-nodes[0].y)/2)+nodes[0].y;
        centerNode.z=((nodes[1].z-nodes[0].z)/2)+nodes[0].z;
        if(scene.getObjectByName("DisconnectIdentifyState")==undefined)this.paintingDisconnectIdentify(centerNode);
        ...
        var pipeSplineout = new THREE.CatmullRomCurve3( [
            centerNode, nodes[1]
        ] );
        var tubeGeometryout = new THREE.TubeBufferGeometry( pipeSplineout, params.line.extrusionSegments, params.line.radius, params.line.radiusSegments, params.line.closed );
        var material=linematerial.clone();
        material.name=lineId+"_"+"near";
        var groupin = THREE.SceneUtils.createMultiMaterialObject( tubeGeometry, [ material ] );
        groupin.name="near";
        material=linematerial.clone();
        material.name=lineId+"_"+"far";
        var groupout = THREE.SceneUtils.createMultiMaterialObject( tubeGeometryout, [ material ] );
        groupout.name="far";
        lineGroup.add(groupin);
        lineGroup.add(groupout);
        lineGroup.name=lineId;
        scene.add(lineGroup);
    },
    ...
    Object.assign(SelectStatesClass.prototype,{
        opacity:1,
        changeColorSpeed:params.pointCircle.changeColorSpeed,
        ...
        stateType:params.pointCircle.loadStateType,
        /**
         * 添加圆选中状态
         * @param circle
         */
        addCircleState:function(topoClass){
               var circle=this;
                ...
                addShape( arcShape,         extrudeSettings,  { color: topoClass.controls.hoverColor, side: THREE.DoubleSide,transparent:true,opacity:params.pointCircle.outSideOpacity },  0,    0, 0, 0, 0, 0, 1 );
                ...
                addShape( arcShape1,         extrudeSettings,  { color: topoClass.controls.hoverColor, side: THREE.DoubleSide,transparent:false },  0,    0, 0, 0, 0, 0, 1 );
                ...
                addShape( arcShape2,         extrudeSettings, { color: topoClass.controls.hoverColor, side: THREE.DoubleSide,transparent:true,opacity:0.4 },  0,    0, 0,0, 0, 0, 1 );
         ...
         * @param circle
         */
        addCubeState:function (topoClass){
            var cubeGeometry=new THREE.BoxGeometry(21,21,21);
            var material=new THREE.MeshBasicMaterial({ color: topoClass.controls.hoverColor, transparent:true,opacity:0.8 });
        ...
        ChangeColor:function(){
            var selectStates=this;
            this.opacity=this.opacity-this.changeColorSpeed;
            if(this.opacity<=0)this.opacity=1;
            this.children[0].material.opacity=this.opacity;
        }
    });
    ...
    SelectStatesClass.prototype.copy = function ( source ) {
        ...
        for(var i=0;i<this.children.length;i++){
            this.children[i].material=source.children[i].material.clone();
        }
    ...
    Object.assign(SearchClass.prototype,{
       ...
       ClearResult : function(){
         ...
         for(var i=0;i<checkArr.length;i++){
             var obj=scene.getObjectByProperty("deviceId","selectState_Identify_Node_"+checkArr[i]);
             scene.remove(obj);
         }
           for(var i=0;i<AlarmArr.length;i++){
               var obj=scene.getObjectByProperty("deviceId","selectState_Alarm_Node_"+checkArr[i]);
               scene.remove(obj);
           }
       },
